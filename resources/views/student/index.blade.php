    ...
    <div class="col-12">
      <div class="card mb-4">
        <div class="card-header pb-0">
          <h6>Students List</h6>
          <a class="btn btn-outline-default btn-sm mb-0 me-3"  href="{{ route('student.create') }}">Add</a>
        </div>
            ...
            <table class="table align-items-center mb-0">
              <thead>
                <tr>
                    ...
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Ad Year</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Parent Name</th>
                    <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">Parent Phone</th>
              ...
              <tbody>
                ...
                <tr class="text-left">
                  <td class="text-center"><p class="text-xs text-secondary mb-0"> {{ $loop->iteration }}</p></td>
                    ...
                    <td>
                      <p class="text-xs text-secondary mb-0">{{ $row->parent_email }}</p>
                    </td>
                    <td class="align-middle">
                      <a href="{{ url('student/edit/'.$row->uuid) }}" class="text-secondary btn btn-sm btn-outline-success font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                        Edit
                      </a>
      ...
      <div class="text-center"> {{ $student->onEachSide(1)->links('vendor.pagination.custom') }} </div>
    </div>
  </div>
...
<script>
  ...
  function more(id){
    $.get('/student/show/' + id, function(response){
